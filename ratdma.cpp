...
void setPublish_data(unsigned char indexAgent, struct _robotData &robotData, int &lostPackets)
{
	switch(indexAgent) 
	{
		case ERSOW_OKTO: 
		{
				...
				pubAgent1Data.ir_detect = robotData.ir_detect;
		...
		case ERSOW_HENDRO: 
		{
				...
				pubAgent2Data.ir_detect = robotData.ir_detect;
		...
		case ERSOW_JAMIL:
		{
				...
				pubAgent3Data.ir_detect = robotData.ir_detect;
...
void *recvThread(void *arg)
{
	...
	while (!end)
	{
		...
		{
			//printf("balldetect: %d<>%d<>%d\n",pubAgent1Data.balldetect,pubAgent2Data.balldetect,pubAgent3Data.balldetect );
			//printf("ir_detect:%d\n",robotData.ir_detect );
			// parsing data header
			...
			printf("got ball detect: %d from robot %d\n", robotData.ball_detect, indexAgent);
